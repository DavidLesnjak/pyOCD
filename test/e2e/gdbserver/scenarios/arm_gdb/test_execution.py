...
def test_software_breakpoint_executes_test_firmware_owned_ram_code(
    ...
    3. Install a software breakpoint at that RAM address before publishing the
...
def test_hardware_breakpoint_stops_and_allows_execution_to_resume(
    ...
    2. Continue to the first exact-address stop and record PC and loop count before
       deleting that breakpoint.
    3. Install a new hardware breakpoint at the same symbol and continue again.
    4. Record the second stop PC and loop count and query GDB's register view.
    5. Require both PCs to equal the symbol and the second loop count to prove fresh
       target progress, then delete all breakpoints and detach.
...
def test_single_step_from_a_known_function_entry(
    ...
    2. Install a breakpoint at ``gdbserver_test_firmware_step_sequence`` before
       submitting the STEP mailbox command.
...
def test_single_step_over_installed_hardware_breakpoint(
    ...
    Test method:
    1. Connect GDB and synchronize at the recurring breakpoint.
    2. Install a hardware breakpoint at the step-sequence entry before submitting
       the STEP mailbox command.
