...
def test_rtt_stream_survives_no_client_connect_and_disconnect(
    ...
    broken.
    """
    _run_transport_stream_lifecycle(gdbserver_gdb, gdbserver_server, MailboxCommand.TRANSPORT_STREAM_RTT, use_rtt=True, use_semihosting=False)
...
def test_semihosting_stream_survives_no_client_connect_and_disconnect(
    ...
    handling is broken.
    """
    _run_transport_stream_lifecycle(gdbserver_gdb, gdbserver_server, MailboxCommand.TRANSPORT_STREAM_SEMIHOSTING, use_rtt=False, use_semihosting=True)
...
def test_combined_transport_stream_survives_no_client_connect_and_disconnect(
    ...
    """
    _run_transport_stream_lifecycle(gdbserver_gdb, gdbserver_server, MailboxCommand.TRANSPORT_STREAM_BOTH, use_rtt=True, use_semihosting=True)
...
def _run_transport_stream_lifecycle(gdb: ExternalGDB, server: PyOCDGDBServer,
    ...
    with ExitStack() as stack:
        ...
        if use_rtt:
            streams.append((b"RTTS", stack.enter_context(server.connect_stream(server.configuration.rtt_port, "arm-gdb-transport-stream-rtt.bin"))))
        if use_semihosting:
            streams.append((b"SEMS", stack.enter_context(server.connect_stream(server.configuration.telnet_port, "arm-gdb-transport-stream-semihosting.bin"))))

        with gdb.start(server, "transport-stream-setup") as setup:
            before_sequence, command_sequence = _queue_transport_stream(setup, command)
            setup.detach()

        initial_sequence = before_sequence + _PHASE_MESSAGE_COUNT
        _wait_for_stream_sequence(streams, initial_sequence)

        with gdb.start_mi(server, "transport-stream-controller") as controller:
            before_connected = _read_transport_stream_state(controller)
            assert before_connected[0] == initial_sequence
            connected_sequence = before_sequence + (2 * _PHASE_MESSAGE_COUNT)
        ...
        with gdb.start_mi(server, "transport-stream-verifier") as verifier:
            _wait_for_command_completion(verifier, command_sequence)
...
def _queue_transport_stream(client: ExternalGDBSession,
                            command: MailboxCommand) -> tuple[int, int]:
    """Synchronize Arm GDB, publish one stream command, and return stream and command sequences."""
    ...
    client.execute("set $gdb_e2e_sequence = gdbserver_test_firmware_mailbox.command_sequence + 1")
    client.execute("set var gdbserver_test_firmware_mailbox.command_argument = %d" % _STREAM_MESSAGE_COUNT)
    client.execute("set var gdbserver_test_firmware_mailbox.command = %d" % int(command))
    client.execute("set var gdbserver_test_firmware_mailbox.command_sequence = $gdb_e2e_sequence")
    output = client.execute('printf "GDB-E2E command-sequence=%u\\n", $gdb_e2e_sequence')
    client.execute("delete breakpoints")
    # Extract the command sequence published immediately before GDB detaches.
    match = re.search(r"GDB-E2E command-sequence=(\d+)", output)
    assert match is not None, output
    return before[0], int(match.group(1))


def _wait_for_command_completion(client: ExternalGDBMISession, command_sequence: int) -> None:
    """Resume a newly attached verifier only until the command state is published.

    Receiving a transport frame only proves that its emit operation began. The
    target may not yet have returned to the dispatcher and written
    ``completed_sequence``. An Arm GDB attach halts the target, so use an
    explicit hardware breakpoint after those writes instead of a timing delay.
    """
    completed = client.evaluate_unsigned("gdbserver_test_firmware_mailbox.completed_sequence")
    if completed == command_sequence:
        return
    ...
    completed = client.evaluate_unsigned("gdbserver_test_firmware_mailbox.completed_sequence")
    assert completed == command_sequence


def _read_transport_stream_state(
        client: ExternalGDBSession | ExternalGDBMISession) -> tuple[int, int, int, int, int]:
    """Read target transport-stream counters through a stopped Arm GDB session."""
    if isinstance(client, ExternalGDBMISession):
        return (
            client.evaluate_unsigned("gdbserver_test_firmware_mailbox.transport_stream_sequence"),
            client.evaluate_unsigned("gdbserver_test_firmware_mailbox.transport_stream_rtt_messages"),
            client.evaluate_unsigned("gdbserver_test_firmware_mailbox.transport_stream_rtt_dropped_bytes"),
            client.evaluate_unsigned("gdbserver_test_firmware_mailbox.transport_stream_semihosting_messages"),
            client.evaluate_unsigned("gdbserver_test_firmware_mailbox.transport_stream_semihosting_failures"),
        )
    ...
    # Extract all five counters from the deterministic transport-stream summary.
    match = re.search(r"GDB-E2E stream=(\d+) rtt=(\d+) dropped=(\d+) semihosting=(\d+) failures=(\d+)", output)
    assert match is not None, output
