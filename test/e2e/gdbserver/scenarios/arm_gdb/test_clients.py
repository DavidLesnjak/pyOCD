...
def test_controller_keeps_spin_after_observer_disconnect(
    ...
    3. Poll advancing SPIN counters through the observer, then detach only that
       observer.
    4. Allow pyOCD to process that detach, interrupt through the original controller,
       and require a different SPIN counter from the observer's earlier snapshot.
    5. Require the server to remain alive and the target to be stopped by that controller.
...
def test_persistent_server_reconnects_after_last_client_disconnect(
    ...
    7. Connect a new GDB/MI process and require a counter different from the
       pre-disconnect snapshot, proving fresh no-client execution of the same SPIN.
    ...
    """
    run_multi_client_workflow("reconnect-" + disconnect, gdbserver_gdb, gdbserver_server)
...
def test_persistent_server_survives_repeated_reconnect_cycles(
    ...
    2. Submit SPIN, use the observer to confirm it has begun executing, and detach
       the observer.
    3. Disconnect the controller gracefully, reconnect, and require fresh SPIN
       progress during the no-client interval.
    4. Before each later resume, connect a temporary observer and require fresh
       target-side SPIN progress after the resume.
    5. Terminate the next GDB abruptly, reconnect again, and require another fresh
       counter value from the same SPIN.
