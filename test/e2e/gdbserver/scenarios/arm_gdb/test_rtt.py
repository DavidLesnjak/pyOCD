...
def test_rtt_symbol_discovery_transfers_output_and_input(
    ...
    6. Compare the target-reported byte count and order-sensitive FNV-1a checksum
       with the exact bytes sent by the host.
    ...
    # Extract the RTT input byte count and checksum reported by the test firmware.
    ...
    assert int(input_state.group(2)) == _rtt_input_checksum(0, 0, _RTT_INPUT), output
