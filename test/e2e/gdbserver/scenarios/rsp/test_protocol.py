...
"""RSP query, memory, and register scenarios for any supported test target."""
...
def test_queries_memory_and_register_write_protocol(
    ...
    try:
        ...
        changed_register_zero = bytes(value ^ 0xA5 for value in original_register_zero)
        raw_rsp_client.write_register(0, changed_register_zero)
        assert raw_rsp_client.read_registers()[:4] == changed_register_zero

        flash_window_address = resolve_elf_symbol(gdbserver_server.configuration.firmware, "gdbserver_test_firmware_flash_window")
        ...
        target = ElementTree.fromstring(target_xml)
        memory_map = ElementTree.fromstring(memory_map_xml)
        threads = ElementTree.fromstring(threads_xml)
        assert _xml_local_name(target.tag) == "target"
        assert any(element.get("name") == "r0" for element in target.iter())
        assert _xml_local_name(memory_map.tag) == "memory-map"
        assert any(element.get("type") == "flash" for element in memory_map.iter())
        assert _xml_local_name(threads.tag) == "threads"
        assert any(_xml_local_name(element.tag) == "thread" for element in threads.iter())
        assert raw_rsp_client.command_response(b"qXfer:features:read:not-target.xml:0,10") == b"E00"
...
@pytest.mark.xfail(strict=True, reason=("pyOCD currently forwards the p register number as bytes instead " "of parsing it"))
