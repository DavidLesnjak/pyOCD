...
_NVIC_ICTR_ADDRESS = 0xE000E004
_NVIC_ICTR_INTLINESNUM_MASK = 0xF
...
def test_wfi_wakes_from_host_pended_nvic_interrupt(
    ...
    3. Read the exact wake IRQ published by the test firmware, read ICTR from the selected core, and require the IRQ to fit that core's implemented NVIC range.
    ...
    with gdbserver_server.connect_rsp() as controller:
        ...
        try:
            ...
            assert wake_irq < _nvic_external_irq_count(controller)
...
def test_hardfault_vector_catch_and_reset_recovery(
    ...
    with gdbserver_server.connect_rsp() as client:
        ...
        try:
            ...
            faulted = mailbox.wait_for(lambda state: state.hardfault_calls == before.hardfault_calls + 1, description="fixture HardFault marker")
...
def _nvic_external_irq_count(client: RSPClient) -> int:
    """Return the external interrupt count implemented by the selected core."""
    interrupt_controller_type = struct.unpack("<I", client.read_memory(_NVIC_ICTR_ADDRESS, 4))[0]
    return ((interrupt_controller_type & _NVIC_ICTR_INTLINESNUM_MASK) + 1) * 32
