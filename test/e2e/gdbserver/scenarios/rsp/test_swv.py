...
"""Opt-in SWV/ITM capture scenarios for any supported SWO-capable target."""
...
def test_swv_raw_stream_captures_test_firmware_itm_data_with_semihosting(
    ...
    Skip: --gdbserver-swv is not enabled; enabled runs also require explicit system and SWO clocks.
    """
    ...
    with gdbserver_server.connect_rsp() as controller:
        with gdbserver_server.connect_rsp() as observer:
            ...
            with gdbserver_server.connect_stream(gdbserver_server.configuration.telnet_port, "swv-semihosting-console.bin") as console:
                with gdbserver_server.connect_stream(gdbserver_server.configuration.swv_raw_port, "swv-raw.bin") as raw_stream:
                    before = mailbox.read()
...
def test_swv_raw_stream_attaches_after_target_execution_begins(
    ...
    with gdbserver_server.connect_rsp() as controller:
        with gdbserver_server.connect_rsp() as observer:
            ...
            controller.send_packet(b"c")
            running = mailbox.wait_for(lambda state: state.heartbeat != before_run.heartbeat, description="fixture execution before raw SWV client attachment")
            ...
            with gdbserver_server.connect_stream(gdbserver_server.configuration.swv_raw_port, "swv-raw-late-client.bin") as raw_stream:
...
def test_swv_raw_stream_drains_after_halt_and_controller_reconnect(
    ...
    try:
        mailbox = _mailbox(gdbserver_server, observer)
        with gdbserver_server.connect_stream(gdbserver_server.configuration.swv_raw_port, "swv-raw-halt-reconnect.bin") as raw_stream:
            ...
            with gdbserver_server.connect_rsp() as reconnected:
                ...
                reconnected_completed = mailbox.wait_for_completion(reconnected_command_sequence)
                reconnected_frame = _fixture_itm_frame(reconnected_completed.itm_sequence)
...
def test_swv_raw_stream_captures_itm_without_a_gdb_client(
    ...
    5. Reconnect a verifier. If trace forwarding preceded command completion, resume
       only to the post-completion hardware breakpoint before checking the mailbox.
    ...
    try:
        ...
        with gdbserver_server.connect_stream(gdbserver_server.configuration.swv_raw_port, "swv-raw-no-gdb.bin") as raw_stream:
            before = mailbox.read()
            command_sequence = mailbox.request(MailboxCommand.ITM_WRITE)
...
def test_swv_raw_consumer_disconnects_and_reconnects(
    ...
    with gdbserver_server.connect_rsp() as controller:
        with gdbserver_server.connect_rsp() as observer:
            mailbox = _mailbox(gdbserver_server, observer)
            with gdbserver_server.connect_stream(gdbserver_server.configuration.swv_raw_port, "swv-raw-consumer-1.bin") as first_stream:
            ...
            with gdbserver_server.connect_stream(gdbserver_server.configuration.swv_raw_port, "swv-raw-consumer-2.bin") as second_stream:
...
@pytest.mark.gdbserver_config(enable_semihosting=True, enable_swv=True, use_incorrect_swv_clock=True)
def test_swv_incorrect_clock_does_not_decode_a_valid_itm_frame(
    ...
    Test method:
    1. Derive a deliberately incorrect SWO receiver rate from the firmware's declared rate and start SWV with it.
    2. Connect controller, observer, and raw SWV clients and record the next expected ITM frame.
    3. Queue ITM_WRITE, continue until the mailbox command completes, and require both target-side ITM counters to advance.
    4. Halt the target and collect any raw bytes that the incorrectly configured probe could recover.
    5. Decode recovered bytes, if any, and assert that the exact sequence-and-checksum frame is absent.
    Expected result: The command completes, while the wrong receiver rate yields either no bytes or data without the valid frame.
    Failure indicates: The target did not execute the ITM write, or a mismatched receiver rate still produced the exact valid frame.
    Skip: --gdbserver-swv is not enabled; enabled runs also require explicit system and SWO clocks.
    """
    configured_clock = request.config.getoption("--gdbserver-swv-clock")
    assert configured_clock is not None
    assert gdbserver_server.configuration.swv_clock != configured_clock
    with gdbserver_server.connect_rsp() as controller:
        with gdbserver_server.connect_rsp() as observer:
            mailbox = _mailbox(gdbserver_server, observer)
            with gdbserver_server.connect_stream(gdbserver_server.configuration.swv_raw_port, "swv-raw-wrong-clock.bin") as raw_stream:
                before = mailbox.read()
                command_sequence = mailbox.request(MailboxCommand.ITM_WRITE)
                expected_frame = _fixture_itm_frame(before.itm_sequence + 1)
                controller.send_packet(b"c")
                ...
                raw_capture = _collect_swv_for(raw_stream, timeout=1.0)
                assert expected_frame not in _decode_itm_port_zero(raw_capture)
...
def _wait_for_itm_frame(stream: TCPStreamClient, expected_frame: bytes,
    ...
    raise AssertionError("SWV raw stream did not decode the expected ITM frame within %.1f seconds: %r" % (timeout, expected_frame))
