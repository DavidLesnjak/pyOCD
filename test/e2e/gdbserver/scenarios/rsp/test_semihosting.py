...
"""Semihosting console and GDB File-I/O scenarios for any supported test target."""
...
from ._completion import (
    command_completion_breakpoint,
    wait_for_command_completion,
    wait_for_resumed_command_completion,
)
...
def test_semihosting_console_is_forwarded_to_telnet(
    ...
    """
    with gdbserver_server.connect_stream(gdbserver_server.configuration.telnet_port, "semihosting-console.bin") as console:
        with gdbserver_server.connect_rsp() as controller:
            with gdbserver_server.connect_rsp() as observer:
                mailbox = _mailbox(gdbserver_server, observer)
                command_sequence = mailbox.request(MailboxCommand.SEMIHOSTING_WRITE)
...
def test_semihosting_console_completes_after_single_step(
    ...
    2. Queue SEMIHOSTING_WRITE, insert Z1 at the function entry, and continue to T05 at that exact PC.
    ...
    """
    breakpoint_address = resolve_elf_symbol(gdbserver_server.configuration.firmware, "gdbserver_test_firmware_semihosting_write") & ~1
    ...
    with gdbserver_server.connect_stream(gdbserver_server.configuration.telnet_port, "semihosting-console-after-step.bin") as console:
        with gdbserver_server.connect_rsp() as controller:
            with gdbserver_server.connect_rsp() as observer:
                mailbox = _mailbox(gdbserver_server, observer)
                before = mailbox.read()
                ...
                try:
                    ...
                    controller.send_packet(b"c")
                    program_counter_before = _expect_sigtrap_at_function_entry(controller, breakpoint_address)
...
def test_semihosting_console_handles_several_requests_in_one_run(
    ...
    """
    request_count = 3
    with gdbserver_server.connect_stream(gdbserver_server.configuration.telnet_port, "semihosting-console-sequence.bin") as console:
        with gdbserver_server.connect_rsp() as controller:
            with gdbserver_server.connect_rsp() as observer:
                mailbox = _mailbox(gdbserver_server, observer)
                before = mailbox.read()
...
def test_semihosting_console_is_serviced_after_final_gdb_detach(
    ...
    5. Reconnect a verifier. If output arrived before the target published completion,
       resume only to the dedicated post-completion hardware breakpoint.
    6. Require exact mailbox completion and one console call.
    ...
    """
    with gdbserver_server.connect_stream(gdbserver_server.configuration.telnet_port, "semihosting-no-client.bin") as console:
        with gdbserver_server.connect_rsp() as controller:
        ...
        with gdbserver_server.connect_rsp() as verifier:
            ...
            completed = wait_for_command_completion(gdbserver_server, verifier, mailbox, command_sequence)
            assert completed.semihosting_console_calls == 1
...
def test_semihosting_console_survives_no_client_connect_and_disconnect(
    ...
    3. Connect one RSP controller and, if the first output preceded completion, resume only to the post-completion hardware breakpoint before checking it.
    4. Queue a second SEMIHOSTING_WRITE and continue it while that one client remains connected. After its message, stop at the completion breakpoint instead of issuing an arbitrary interrupt.
    ...
    6. Reconnect a verifier and use the completion breakpoint if needed before requiring three console calls, proving each lifecycle phase produced one message without duplication or loss.
    ...
    """
    with gdbserver_server.connect_stream(gdbserver_server.configuration.telnet_port, "semihosting-client-lifecycle.bin") as console:
        ...
        with gdbserver_server.connect_rsp() as controller:
            mailbox = _mailbox(gdbserver_server, controller)
            no_client_completed = wait_for_command_completion(gdbserver_server, controller, mailbox, no_client_sequence)
            assert no_client_completed.semihosting_console_calls == (
                before.semihosting_console_calls + 1)

            connected_sequence = mailbox.request(MailboxCommand.SEMIHOSTING_WRITE)
            with command_completion_breakpoint(gdbserver_server, controller):
                controller.send_packet(b"c")
                captured = _wait_for_console_messages(console, 2)
                assert captured.count(_CONSOLE_MESSAGE) == 2
                connected_completed = wait_for_resumed_command_completion(controller, mailbox, connected_sequence)
        ...
        with gdbserver_server.connect_rsp() as verifier:
            mailbox = _mailbox(gdbserver_server, verifier)
            disconnected_completed = wait_for_command_completion(gdbserver_server, verifier, mailbox, disconnected_sequence)
...
def test_semihosting_console_works_after_rsp_reset_and_reconnect(
    ...
    3. Reconnect the controller, install a hardware breakpoint at the recurring test-firmware site, and continue until post-reset initialization reaches that breakpoint.
    4. Connect an observer while the target is halted, require the exact next boot epoch and a zero console counter, then attach a new telnet stream.
    5. Queue SEMIHOSTING_WRITE while halted, install the command-completion breakpoint, continue, capture exactly one message, and verify completion after the breakpoint stops the target.
    ...
    """
    mailbox_address = resolve_elf_symbol(gdbserver_server.configuration.firmware, "gdbserver_test_firmware_mailbox")
    ...
    with gdbserver_server.connect_rsp() as reconnected:
        _resume_to_test_firmware_breakpoint(gdbserver_server, reconnected)
        with gdbserver_server.connect_rsp() as observer:
            ...
            with gdbserver_server.connect_stream(gdbserver_server.configuration.telnet_port, "semihosting-console-after-reset.bin") as console:
                command_sequence = mailbox.request(MailboxCommand.SEMIHOSTING_WRITE)
                with command_completion_breakpoint(gdbserver_server, reconnected):
                    reconnected.send_packet(b"c")
                    captured = _wait_for_console_messages(console, 1)
                    completed = wait_for_resumed_command_completion(reconnected, mailbox, command_sequence)
                assert captured.count(_CONSOLE_MESSAGE) == 1
                assert completed.semihosting_console_calls == 1
...
def test_gdb_file_syscalls_complete_after_single_step(
    ...
    3. Continue to the breakpoint, verify SIGTRAP at the exact entry PC, then remove the breakpoint.
    ...
    """
    breakpoint_address = resolve_elf_symbol(gdbserver_server.configuration.firmware, "gdbserver_test_firmware_semihosting_file_write") & ~1
    insert_packet = _breakpoint_packet(b"Z1", breakpoint_address)
    remove_packet = _breakpoint_packet(b"z1", breakpoint_address)
    breakpoint_inserted = False
...
@pytest.mark.gdbserver_config(enable_semihosting=True, semihost_use_syscalls=True, extra_arguments=("-vv",))
def test_gdb_file_syscalls_fail_without_an_active_client(
    ...
    4. Reconnect a verifier. If the no-client diagnostic preceded command completion,
       resume only to the post-completion hardware breakpoint rather than deadlock.
    5. Require one failed open result and the host platform's exact ENOTCONN value.
    ...
    assert "Skipping GDB syscall because no client is available" in gdbserver_server.wait_for_log("Skipping GDB syscall because no client is available")
    with gdbserver_server.connect_rsp() as verifier:
        mailbox = _mailbox(gdbserver_server, verifier)
        completed = wait_for_command_completion(gdbserver_server, verifier, mailbox, command_sequence)
        assert completed.semihosting_file_calls == 1
        assert completed.semihosting_open_result_signed == -1
        assert completed.semihosting_errno == ENOTCONN
...
def test_gdb_file_syscall_recovers_when_active_client_disconnects(
    ...
    4. Use the observer to require command completion with a failed open result and exact ENOTCONN errno.
    ...
    assert "Connection closed during syscall" in gdbserver_server.wait_for_log("Connection closed during syscall")
...
def _resume_to_test_firmware_breakpoint(server: PyOCDGDBServer,
                                        client: RSPClient) -> None:
    """Run post-reset initialization to the recurring test-firmware breakpoint."""
    breakpoint_address = resolve_elf_symbol(server.configuration.firmware, "gdbserver_test_firmware_breakpoint_site") & ~1
    insert_packet = _breakpoint_packet(b"Z1", breakpoint_address)
    remove_packet = _breakpoint_packet(b"z1", breakpoint_address)
    assert client.command(insert_packet) == b"OK"
    try:
        client.send_packet(b"c")
        _expect_sigtrap_at_function_entry(client, breakpoint_address)
    finally:
...
def _expect_sigtrap_at_function_entry(client: RSPClient, address: int) -> int:
    """Require a hardware-breakpoint stop at the exact test-firmware function entry."""
