...
"""RTT discovery, transport, and input scenarios for any supported test target."""
...
def test_rtt_symbol_discovery_serves_initial_frame_without_gdb_client(
    ...
    with gdbserver_server.connect_stream(gdbserver_server.configuration.rtt_port, "rtt-no-gdb.bin") as stream:
...
def test_rtt_symbol_discovery_transfers_output_and_input(
    ...
    with gdbserver_server.connect_rsp() as controller:
        with gdbserver_server.connect_rsp() as observer:
            ...
            with gdbserver_server.connect_stream(gdbserver_server.configuration.rtt_port, "rtt-symbol.bin") as stream:
                before = mailbox.read()
                ...
                _assert_ordered_complete_frame_range(captured, completed.rtt_sequence, completed.rtt_sequence)

                down_data = b"pyocd-rtt-input-0123456789"
                expected_bytes = completed.rtt_input_bytes + len(down_data)
                expected_checksum = _rtt_input_checksum(completed.rtt_input_checksum, completed.rtt_input_bytes, down_data)
                ...
                _assert_ordered_complete_frame_range(captured, completed.rtt_sequence + 1, input_received.rtt_sequence)
...
def test_rtt_burst_channel_preserves_high_rate_framed_output(
    ...
    with gdbserver_server.connect_rsp() as controller:
        with gdbserver_server.connect_rsp() as observer:
            mailbox = _mailbox(gdbserver_server, observer)
            with gdbserver_server.connect_stream(gdbserver_server.configuration.rtt_burst_port, "rtt-burst.bin") as stream:
                before = mailbox.read()
                command_sequence = mailbox.request(MailboxCommand.RTT_BURST, argument=_BURST_FRAME_COUNT)
                controller.send_packet(b"c")
                completed = mailbox.wait_for_completion(command_sequence)

                captured = _wait_for_rtt_burst_sequence(stream, completed.rtt_burst_sequence)
                ...
                _assert_ordered_complete_burst_frame_range(captured, first_burst_sequence, completed.rtt_burst_sequence)
                controller.interrupt()
                assert controller.receive_packet(timeout=5.0).startswith(b"T02")
...
def test_rtt_output_is_drained_after_target_halt(
    ...
    with gdbserver_server.connect_rsp() as controller:
        with gdbserver_server.connect_rsp() as observer:
            mailbox = _mailbox(gdbserver_server, observer)
            with gdbserver_server.connect_stream(gdbserver_server.configuration.rtt_port, "rtt-after-halt.bin") as stream:
                before = mailbox.read()
                command_sequence = mailbox.request(MailboxCommand.RTT_WRITE)
                controller.send_packet(b"c")
                completed = mailbox.wait_for_completion(command_sequence)
...
def test_rtt_explicit_control_block_address_reconnects_stream(
    ...
    """
    expected_address = resolve_elf_symbol(gdbserver_server.configuration.firmware, "_SEGGER_RTT")
    ...
    with gdbserver_server.connect_rsp() as controller:
        with gdbserver_server.connect_rsp() as observer:
            mailbox = _mailbox(gdbserver_server, observer)
            with gdbserver_server.connect_stream(gdbserver_server.configuration.rtt_port, "rtt-address-first.bin") as stream:
                before = mailbox.read()
                command_sequence = mailbox.request(MailboxCommand.RTT_WRITE)
                controller.send_packet(b"c")
                completed = mailbox.wait_for_completion(command_sequence)
            ...
            with gdbserver_server.connect_stream(gdbserver_server.configuration.rtt_port, "rtt-address-reconnected.bin") as stream:
                before = mailbox.read()
                command_sequence = mailbox.request(MailboxCommand.RTT_WRITE)
                completed = mailbox.wait_for_completion(command_sequence)
                captured = _wait_for_rtt_sequence(stream, completed.rtt_sequence)
...
def test_active_rtt_stream_survives_final_gdb_disconnect_and_reconnect(
    ...
    4. Connect a replacement RSP client. If the frame preceded mailbox completion,
       resume only to the post-completion hardware breakpoint, then verify counters.
    5. Queue another RTT_WRITE, resume with the replacement client, and require the next frame on the original RTT socket.
    6. Stop at the post-completion hardware breakpoint and verify completion without reconnecting RTT.
    ...
    try:
        ...
        with gdbserver_server.connect_stream(gdbserver_server.configuration.rtt_port, "rtt-last-gdb-client.bin") as stream:
            ...
            captured = _wait_for_new_rtt_sequence(stream, expected_sequence, stream_offset)
            _assert_ordered_complete_frame_range(captured, expected_sequence, expected_sequence)
            ...
            with gdbserver_server.connect_rsp() as reconnected:
                ...
                completed = wait_for_command_completion(gdbserver_server, reconnected, mailbox, command_sequence)
                ...
                with command_completion_breakpoint(gdbserver_server, reconnected):
                    ...
                    captured = _wait_for_new_rtt_sequence(stream, expected_reconnected_sequence, stream_offset)
                    completed = wait_for_resumed_command_completion(reconnected, mailbox, reconnected_sequence)
                ...
                _assert_ordered_complete_frame_range(captured, expected_reconnected_sequence, expected_reconnected_sequence)
    finally:
        controller.close()
...
def test_active_rtt_stream_rediscovers_after_target_reset_and_gdb_reconnect(
    ...
    6. Queue RTT_WRITE, continue, and require sequence 2 on the original socket.
    7. Stop at the post-completion hardware breakpoint before inspecting completion.
    ...
    try:
        ...
        with gdbserver_server.connect_stream(gdbserver_server.configuration.rtt_port, "rtt-active-reset.bin") as stream:
            _wait_for_rtt_sequence(stream, 1)
            stream_offset = len(stream.received)
...
def _rtt_input_checksum(previous: int, previous_bytes: int, data: bytes) -> int:
    """Calculate the target firmware's incremental, order-sensitive FNV-1a hash."""
    checksum = previous if previous_bytes else 2166136261
    for value in data:
        checksum ^= value
...
def _wait_for_new_rtt_sequence(stream: TCPStreamClient, sequence: int,
    ...
    while time.monotonic() < deadline:
        ...
        if sequence in _low_rate_frame_sequences(new_captured):
            return new_captured
        stream.read_available(timeout=min(0.200, max(0.001, deadline - time.monotonic())))
    raise AssertionError("new RTT frame %d did not arrive within %.1f seconds" % (sequence, timeout))
...
def _wait_for_rtt_burst_sequence(stream: TCPStreamClient, sequence: int,
    ...
    while time.monotonic() < deadline:
        ...
        if sequence in _burst_frame_sequences(captured):
            return captured
        stream.read_available(timeout=min(0.200, max(0.001, deadline - time.monotonic())))
    raise AssertionError("RTT burst frame %d did not arrive within %.1f seconds" % (sequence, timeout))
