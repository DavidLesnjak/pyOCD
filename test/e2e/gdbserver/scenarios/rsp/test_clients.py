...
def test_persistent_server_reconnects_after_last_client_disconnect(
    ...
    Test method:
    1. Connect the controller and queue SPIN, then use a temporary observer to prove that continue was dispatched and SPIN advanced.
    ...
    4. Connect a replacement RSP client, which halts the target, and prove that SPIN advanced again during the no-client interval.
    ...
    try:
        ...
        mailbox.wait_until_ready()
        sequence = mailbox.request(MailboxCommand.SPIN)
        before_disconnect_iterations = _continue_and_observe_spin(gdbserver_server, controller, mailbox_address, sequence, 0)
    ...
    with gdbserver_server.connect_rsp() as reconnected:
        ...
        assert running.spin_iterations != before_disconnect_iterations
...
def test_persistent_server_survives_repeated_reconnect_cycles(
    ...
    Test method:
    1. Start one SPIN command and use a temporary observer to prove each all-stop continue was dispatched and advanced SPIN.
    ...
    4. Read SPIN progress after each reconnect and require it to differ from the last observer-confirmed pre-disconnect value.
    ...
    try:
        ...
        for disconnect in ("graceful", "abrupt", "graceful"):
            before_disconnect_iterations = _continue_and_observe_spin(gdbserver_server, client, mailbox_address, sequence, previous_iterations)
            if disconnect == "graceful":
                client.interrupt()
            ...
            assert reconnected.spin_iterations != before_disconnect_iterations
...
def _continue_and_observe_spin(
        server: PyOCDGDBServer,
        controller: RSPClient,
        mailbox_address: int,
        sequence: int,
        previous_iterations: int) -> int:
    """Continue through one controller and prove SPIN advances through an observer."""
    with server.connect_rsp() as observer:
        observer_mailbox = FixtureMailboxClient(observer, mailbox_address)
        controller.send_packet(b"c")
        running = observer_mailbox.wait_for(
            lambda state: (state.command_sequence == sequence and
                           state.spin_state == MailboxSpinState.RUNNING and
                           state.spin_iterations != previous_iterations),
            description="fixture spin progress before client disconnect")
        return running.spin_iterations
...
def test_server_recovers_after_dispatched_single_step_client_disconnect(
    ...
    2. Continue to T05, verify the exact entry PC, and remove the breakpoint before dispatching the raw s packet.
    ...
    try:
        ...
        observer_mailbox = FixtureMailboxClient(observer, mailbox_address)
        controller_mailbox.wait_until_ready()
        sequence = controller_mailbox.request(MailboxCommand.STEP, argument=step_argument)
        assert controller.command(insert_packet) == b"OK"
        breakpoint_inserted = True
        controller.send_packet(b"c")
        assert controller.receive_packet(timeout=5.0).startswith(b"T05")
        program_counter_before = _program_counter(controller)
        assert program_counter_before == breakpoint_address
        assert controller.command(remove_packet) == b"OK"
        breakpoint_inserted = False
        ...
        controller.close()
        stepped_program_counter = _wait_for_program_counter_change(observer, program_counter_before)
