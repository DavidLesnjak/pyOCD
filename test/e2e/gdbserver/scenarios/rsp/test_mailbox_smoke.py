...
"""Initial single-client mailbox and RSP access scenario for any supported test target."""
...
def test_mailbox_memory_registers_and_spin(fixture_mailbox: FixtureMailboxClient,
    ...
    original_window = raw_rsp_client.read_memory(fixture_mailbox.address + MAILBOX_HEADER_SIZE, 16)
    ...
    assert raw_rsp_client.read_memory(fixture_mailbox.address + MAILBOX_HEADER_SIZE, len(test_window)) == test_window
