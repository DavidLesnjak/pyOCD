...
"""Hardware-breakpoint capacity scenario for any supported test target."""
...
_BREAKPOINT_CATALOG_SYMBOLS = tuple("gdbserver_test_firmware_breakpoint_catalog_%02d" % index for index in range(24))
...
def test_hardware_breakpoint_capacity_reports_exhaustion_and_recovers(
    ...
    if breakpoint_count >= len(_BREAKPOINT_CATALOG_SYMBOLS):
        pytest.skip("fixture has %d catalog locations but target reports %d hardware breakpoints" % (len(_BREAKPOINT_CATALOG_SYMBOLS), breakpoint_count))
    ...
    with gdbserver_server.connect_rsp() as observer:
        ...
        try:
            ...
            for address in breakpoint_addresses:
                response = raw_rsp_client.command_response(_breakpoint_packet(b"Z1", address))
                if response == b"OK":
            ...
            assert _program_counter(raw_rsp_client) == breakpoint_addresses[0]
...
def _reported_hardware_breakpoint_count(client: RSPClient) -> int:
    ...
    control = int.from_bytes(client.read_memory(_FPB_CONTROL_ADDRESS, 4), byteorder="little")
