...
"""Initial no-client gdbserver scenario for any supported test target."""
...
def test_server_runs_test_firmware_before_a_gdb_client_connects(gdbserver_server: PyOCDGDBServer) -> None:
    ...
    Expected result: The server remains alive, metadata names the cbuild target, and heartbeat/loop counts are non-zero.
    ...
    assert gdbserver_server.configuration.cbuild_target_type
    assert run_metadata["cbuild_target_type"] == (
        gdbserver_server.configuration.cbuild_target_type)
