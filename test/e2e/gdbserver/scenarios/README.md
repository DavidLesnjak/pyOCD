...
The server remains alive, metadata names the cbuild target, and heartbeat/loop counts are non-zero.
...
#### Range step stops at installed hardware breakpoint

- Exact test: `test/e2e/gdbserver/scenarios/rsp/test_execution.py::test_range_step_stops_at_installed_hardware_breakpoint`
...
The first range step stops exactly at hardware breakpoint X and the second reaches exclusive endpoint B.

**Failure indicates**

Range stepping ignores an FPB event, reports the wrong PC, or cannot resume after breakpoint removal.

#### Range step stops at installed software breakpoint

- Exact test: `test/e2e/gdbserver/scenarios/rsp/test_execution.py::test_range_step_stops_at_installed_software_breakpoint`
...
The first range step stops exactly at software breakpoint X and the second reaches exclusive endpoint B.

**Failure indicates**

Range stepping misses a managed Z0 patch, reports the wrong PC, or leaves executable RAM modified.

#### Range step stops at literal bkpt instruction

- Exact test: `test/e2e/gdbserver/scenarios/rsp/test_execution.py::test_range_step_stops_at_literal_bkpt_instruction`
...
The literal BKPT terminates the first range step inside the interval and execution then reaches B without retriggering.
...
--gdbserver-swv is not enabled; enabled runs also require explicit system and SWO clocks.
...
The command completes, while the wrong receiver rate yields either no bytes or data without the valid frame.

**Failure indicates**

The target did not execute the ITM write, or a mismatched receiver rate still produced the exact valid frame.
...
Each flash packet returns OK, readback equals the payload, the guard remains erased, and cleanup reads as erased bytes.
...
GDB reports the test-firmware AXF, symbols, registers, selected core, breakpoint stops, and detach.
...
Vector catch reports the deliberate HardFault through GDB and monitor reset publishes exactly one new debuggable firmware generation.
