...
class ExternalGDBSession:
    ...
    def _send(self, data: bytes) -> None:
        ...
        try:
            ...
            raise ExternalGDBError("external GDB closed unexpectedly; see %s" % self._output_path) from error
    ...
    def _wait_for(self, marker: str, timeout: float) -> None:
        ...
        while True:
            ...
            if self._process.poll() is not None:
                raise ExternalGDBError("external GDB exited with code %d; see %s" % (self._process.returncode, self._output_path))
            remaining = deadline - time.monotonic()
            if remaining <= 0:
                raise ExternalGDBError("external GDB did not complete %r; see %s" % (marker, self._output_path))
...
class ExternalGDBMISession:
    ...
    def console(self, command: str, timeout: float = 10.0) -> str:
        ...
        return self.command("-interpreter-exec console %s" % _mi_quote(command), timeout)
    ...
    def evaluate_unsigned(self, expression: str, timeout: float = 10.0) -> int:
        ...
        output = self.command("-data-evaluate-expression %s" % _mi_quote(expression), timeout)
        # Extract the decimal value from GDB/MI's successful expression result.
        value = re.search(r'\^done,value="([0-9]+)"', output)
        if value is None:
            raise ExternalGDBError("external GDB/MI did not return an unsigned value for %r; see %s" % (expression, self._output_path))
    ...
    def _send(self, data: bytes) -> None:
        ...
        try:
            ...
            raise ExternalGDBError("external GDB/MI closed unexpectedly; see %s" % self._output_path) from error
    ...
    def _wait_for_result(self, token: int, expected_result: str,
                         timeout: float) -> str:
        # Match this command token's complete GDB/MI result record.
    ...
    def _wait_for_stop(self, offset: int, timeout: float) -> str:
        ...
        while True:
            ...
            # Detect the asynchronous GDB/MI event that reports a target stop.
    ...
    def _wait_for_output(self, deadline: float, description: str) -> None:
        if self._process.poll() is not None:
            raise ExternalGDBError("external GDB/MI exited with code %d; see %s" % (self._process.returncode, self._output_path))
        remaining = deadline - time.monotonic()
        if remaining <= 0:
            raise ExternalGDBError("external GDB/MI did not complete %s; see %s" % (description, self._output_path))
        self._output_event.wait(min(remaining, 0.100))
        self._output_event.clear()
...
def _safe_artifact_name(value: str) -> str:
    ...
    # Replace every run of filesystem-unsafe characters with one separator.
...
class ExternalGDB:
    ...
    def run(self, server: PyOCDGDBServer, commands: Sequence[str],
        ...
        with output_path.open("wb") as output:
            ...
            try:
                ...
                raise ExternalGDBError("external GDB timed out; see %s" % output_path) from error
            except OSError as error:
                raise ExternalGDBError("could not run --gdbserver-gdb %r: %s" % (self._executable, error)) from error

        output_text = output_path.read_text(encoding="utf-8", errors="replace")
        if completed.returncode != 0:
            raise ExternalGDBError("external GDB exited with code %d; see %s" % (completed.returncode, output_path))
        return output_text
    ...
    def _get_version(self, cwd: Path, timeout: float) -> str:
        ...
        try:
            ...
            raise ExternalGDBError("external GDB version query timed out for %r" % self._executable) from error
        except OSError as error:
            raise ExternalGDBError("could not run --gdbserver-gdb %r: %s" % (self._executable, error)) from error
        if completed.returncode != 0:
            raise ExternalGDBError("external GDB version query exited with code %d for %r" % (completed.returncode, self._executable))
...
def pytest_addoption(parser: pytest.Parser) -> None:
    ...
    group.addoption("--gdbserver-e2e", action="store_true", default=False, help="enable tests that require an attached dedicated hardware target")
    group.addoption("--gdbserver-probe-uid", default=None, metavar="UID", help="unique identifier of the exclusive debug probe")
    group.addoption("--gdbserver-firmware", default=None, metavar="AXF", help="optional AXF/ELF override; normally derived from the cbuild-run file")
    group.addoption("--gdbserver-cbuild-run", default=None, metavar="PATH", help="generated CMSIS cbuild-run.yml describing the target and debug configuration")
    group.addoption("--gdbserver-mailbox-symbol", default="gdbserver_test_firmware_mailbox", metavar="SYMBOL", help="fixture mailbox ELF symbol")
    group.addoption("--gdbserver-artifacts", default=None, metavar="DIRECTORY", help="directory for generated run artifacts")
    group.addoption("--gdbserver-port", type=int, default=0, metavar="PORT", help="GDB server port; 0 allocates an available local port")
    group.addoption("--gdbserver-telnet-port", type=int, default=0, metavar="PORT", help="semihosting telnet port; 0 allocates an available local port")
    group.addoption(
        "--gdbserver-extra-arg",
        action="append",
        default=[],
    ...
    group.addoption("--gdbserver-no-program", action="store_true", default=False, help="do not flash the fixture before each scenario")
    group.addoption("--gdbserver-no-reset-run", action="store_true", default=False, help="do not pass --reset-run when starting pyOCD gdbserver")
    group.addoption("--gdbserver-semihosting", action="store_true", default=False, help="enable pyOCD semihosting for the scenario")
    group.addoption("--gdbserver-swv", action="store_true", default=False, help="enable hardware SWV capture scenarios after verifying the board SWO route")
    group.addoption("--gdbserver-swv-system-clock", type=int, default=None, metavar="HERTZ", help="actual target system clock used by the SWV test firmware")
    group.addoption("--gdbserver-swv-clock", type=int, default=None, metavar="HERTZ", help="SWO baud rate produced by the selected target firmware")
    group.addoption(
        "--gdbserver-gdb",
        default=None,
        metavar="EXECUTABLE",
...
def pytest_configure(config: pytest.Config) -> None:
    ...
    config.addinivalue_line("markers", "gdbserver_arm_gdb: scenario driven by arm-none-eabi-gdb")
...
def pytest_collection_modifyitems(config: pytest.Config,
    ...
    if not config.getoption("--gdbserver-gdb"):
        skip_external_gdb = pytest.mark.skip(reason=("external GDB scenario disabled; pass --gdbserver-gdb with the " "intended executable"))
    for item in items:
        ...
        if "scenarios" in scenario_parts:
            if "rsp" in scenario_parts:
                ...
                _validate_collected_scenario(item)
        if (skip_external_gdb is not None and item.get_closest_marker("gdbserver_external_gdb") is not None):
...
def gdbserver_gdb(request: pytest.FixtureRequest) -> ExternalGDB:
    ...
    if not executable:
        pytest.skip("external GDB scenario disabled; pass --gdbserver-gdb with the " "intended executable")
...
def gdbserver_flash_scratch(request: pytest.FixtureRequest) -> tuple[int, int]:
    ...
    if address_option is None or size_option is None:
        pytest.skip("flash-protocol test requires --gdbserver-flash-scratch-address and " "--gdbserver-flash-scratch-size for a reserved region")
    try:
        address = int(address_option, 0)
        size = int(size_option, 0)
    except ValueError as error:
        raise pytest.UsageError("flash scratch address and size must be decimal or 0x-prefixed integers") from error
...
def _configuration_for_test(request: pytest.FixtureRequest) -> GDBServerConfiguration:
    ...
    if enable_swv and not config.getoption("--gdbserver-swv"):
        pytest.skip("SWV capture is capability-gated; pass --gdbserver-swv only after verifying " "the selected board's SWO route and probe support")

    swv_system_clock = settings.get("swv_system_clock", config.getoption("--gdbserver-swv-system-clock"))
    swv_clock = settings.get("swv_clock", config.getoption("--gdbserver-swv-clock"))
    if enable_swv and swv_system_clock is None:
        raise pytest.UsageError("SWV scenarios require --gdbserver-swv-system-clock with the test " "firmware's actual runtime clock")
    if enable_swv and swv_clock is None:
        raise pytest.UsageError("SWV scenarios require --gdbserver-swv-clock with the test firmware's " "configured SWO baud rate")
    if swv_system_clock is not None and swv_system_clock <= 0:
        raise pytest.UsageError("--gdbserver-swv-system-clock must be positive")
    if swv_clock is not None and swv_clock <= 0:
        raise pytest.UsageError("--gdbserver-swv-clock must be positive")
    if settings.get("use_incorrect_swv_clock", False):
        if "swv_clock" in settings:
            raise pytest.UsageError("gdbserver_config cannot combine swv_clock with use_incorrect_swv_clock")
        if swv_clock is None:
            raise pytest.UsageError("gdbserver_config use_incorrect_swv_clock requires " "--gdbserver-swv-clock")
        swv_clock = _incorrect_swv_clock(swv_clock)
...
def _scenario_configuration(request: pytest.FixtureRequest) -> dict[str, Any]:
    ...
    if unsupported:
        raise pytest.UsageError("unsupported gdbserver_config setting(s): %s" % ", ".join(sorted(unsupported)))
    for name in (
            "enable_semihosting",
            "enable_swv",
            "persist",
...
def _incorrect_swv_clock(swv_clock: int) -> int:
    """Return a positive clock that deliberately differs from the supplied SWO rate."""
    incorrect_clock = max(1, swv_clock * 3 // 4)
    return incorrect_clock if incorrect_clock != swv_clock else swv_clock + 1
