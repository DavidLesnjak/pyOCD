...
def test_artifact_storage_creates_a_distinct_run_directory(
    ...
    artifacts = RunArtifacts.create(tmp_path, "B-U585I-IOT02A", "mailbox smoke")
...
def test_mailbox_reset_ready_requires_new_idle_generation() -> None:
    ...
    active_mailbox = FixtureMailbox.from_bytes(struct.pack(MAILBOX_HEADER_FORMAT, *header) + bytes(MAILBOX_RAM_WINDOW_SIZE))
...
def test_rsp_client_preserves_non_stop_notification_packet_type() -> None:
    ...
    def _serve(connection: socket.socket) -> None:
        ...
        connection.sendall(b"+" + _packet(b"Stop:T0507:00100020;", packet_type=b"%"))
        assert _receive_exact(connection, 1) == b"+"
...
def test_rsp_client_defers_notification_that_arrives_before_command_reply() -> None:
    ...
    def _serve(connection: socket.socket) -> None:
        assert _packet_payload(_receive_packet(connection)) == b"qC"
        connection.sendall(_packet(b"Stop:T0507:00100020;", packet_type=b"%") + b"+" + _packet(b"QC1"))
        assert _receive_exact(connection, 1) == b"+"
        assert _receive_exact(connection, 1) == b"+"
...
def test_rsp_client_uses_unescaped_qxfer_length_for_the_next_offset() -> None:
    ...
    with _rsp_server(_serve) as port:
        with RSPClient.connect("127.0.0.1", port) as client:
            assert client.read_xfer("features", "target.xml", chunk_size=8) == b"A#$}*"
...
def test_gdbserver_configuration_serializes_numeric_session_options(
    ...
    configuration = _configuration(tmp_path, enable_swv=True, swv_system_clock=160000000, swv_clock=2000000, vector_catch="h")
...
@pytest.mark.parametrize("clock_arguments", ({"swv_system_clock": 160000000}, {"swv_clock": 2000000},), ids=("missing-swo-clock", "missing-system-clock"))
def test_gdbserver_configuration_requires_explicit_swv_clocks(
        tmp_path: Path, clock_arguments: dict[str, int]) -> None:
    """SWV setup never substitutes a device maximum or board-specific default."""
    with pytest.raises(ValueError):
        _configuration(tmp_path, enable_swv=True, **clock_arguments)
...
def test_gdbserver_configuration_builds_address_rtt_option(
    ...
    monkeypatch.setattr(pyocd_server, "resolve_elf_symbol", lambda firmware, symbol: 0x20001234)
...
def test_gdbserver_configuration_records_both_rtt_channel_ports(
    ...
    metadata = json.loads((configuration.artifacts.directory / "run.json").read_text(encoding="utf-8"))
    ...
    assert metadata["cbuild_target_type"] == "unit-test-target"
...
def test_program_firmware_retries_the_observed_transient_usb_timeout(
        tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    """A transient probe handover failure is retried with a separate log per attempt."""
    ...
    def _run(*args: object, **kwargs: object) -> subprocess.CompletedProcess:
        ...
        if run_count == 1:
            output.write(b"USB Error: [Errno 10060] Operation timed out\n")
            return subprocess.CompletedProcess(args, 1)
        output.write(b"Programmed successfully\n")
        return subprocess.CompletedProcess(args, 0)

    monkeypatch.setattr(pyocd_server.subprocess, "run", _run)
    monkeypatch.setattr(pyocd_server.time, "sleep", retry_delays.append)

    server.program_firmware()

    assert run_count == 2
    assert retry_delays == [server._PROGRAM_RETRY_DELAY]
    assert (configuration.artifacts.directory / "program-attempt-1.log").read_bytes() == (
        b"USB Error: [Errno 10060] Operation timed out\n")
    assert (configuration.artifacts.directory / "program-attempt-2.log").read_bytes() == (
        b"Programmed successfully\n")


def test_program_firmware_does_not_retry_a_non_transient_failure(
        tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    """A normal pyOCD load failure remains visible immediately instead of being retried."""
    configuration = _configuration(tmp_path)
    server = PyOCDGDBServer(configuration)
    run_count = 0
    ...
    def _run(*args: object, **kwargs: object) -> subprocess.CompletedProcess:
        ...
        output = kwargs["stdout"]
        assert hasattr(output, "write")
        output.write(b"Error: selected flash algorithm was not found\n")
        return subprocess.CompletedProcess(args, 1)

    monkeypatch.setattr(pyocd_server.subprocess, "run", _run)

    with pytest.raises(pyocd_server.FirmwareProgrammingError, match="exit code 1"):
        server.program_firmware()

    assert run_count == 1
    assert (configuration.artifacts.directory / "program-attempt-1.log").read_bytes() == (
        b"Error: selected flash algorithm was not found\n")
    assert not (configuration.artifacts.directory / "program-attempt-2.log").exists()
...
def test_gdbserver_records_rsp_connection_identity(
    ...
    configuration = _configuration(tmp_path)
    server = PyOCDGDBServer(configuration)
    server._record_rsp_packet(7, RSPPacket(1.0, "receive", "$", b"qC"))

    record = json.loads((configuration.artifacts.directory / "rsp.jsonl").read_text(encoding="utf-8"))
...
def test_tcp_stream_client_collects_fragmented_cumulative_data() -> None:
    ...
    with _tcp_stream_server(_serve) as port:
        with TCPStreamClient.connect("127.0.0.1", port, data_observer=observed.append) as stream:
...
def _configuration(tmp_path: Path, **overrides: object) -> GDBServerConfiguration:
    ...
    cbuild_run.write_text(
        "cbuild-run:\n"
        "  target-type: unit-test-target\n"
        "  system-resources:\n"
        "    processors:\n"
        "      - max-clock: 160000000\n",
        encoding="utf-8")
