...
class GDBServerConfiguration:
    ...
    cbuild_target_type: Optional[str] = field(init=False, default=None)
    ...
    def __post_init__(self) -> None:
        ...
        cbuild_configuration = _read_cbuild_run(self.cbuild_run)
        target_type = cbuild_configuration.get("target-type")
        self.cbuild_target_type = target_type if isinstance(target_type, str) else None
        ...
        if self.enable_swv:
            ...
            self.enable_semihosting = True
            if self.swv_system_clock is None:
                raise ValueError("SWV system clock must be supplied from the test firmware's " "actual runtime configuration")
            if self.swv_clock is None:
                raise ValueError("SWV clock must be supplied from the test firmware's configured " "SWO baud rate")
            ...
            if self.swv_raw_port == 0:
                ...
                self.swv_raw_port = _find_free_tcp_port((self.gdb_port, self.telnet_port) + rtt_ports)
...
class PyOCDGDBServer:
    ...
    _PROGRAM_ATTEMPTS = 3
    _PROGRAM_RETRY_DELAY = 1.0
    _TRANSIENT_PROGRAM_FAILURE = "USB Error: [Errno 10060] Operation timed out"
    ...
    def program_firmware(self) -> None:
        ...
        for attempt in range(1, self._PROGRAM_ATTEMPTS + 1):
            output_path = self.configuration.artifacts.directory / (
                "program-attempt-%d.log" % attempt)
            with output_path.open("wb") as output:
                try:
                    ...
                    raise FirmwareProgrammingError("fixture programming timed out; see %s" % output_path) from error

            if completed.returncode == 0:
                return
            if not self._is_transient_program_failure(output_path):
                raise FirmwareProgrammingError("fixture programming failed with exit code %d; see %s" % (completed.returncode, output_path))
            if attempt == self._PROGRAM_ATTEMPTS:
                raise FirmwareProgrammingError("fixture programming failed after %d transient USB timeouts; see %s" % (attempt, output_path))
            time.sleep(self._PROGRAM_RETRY_DELAY)
    ...
    def start(self) -> None:
        ...
        self._process = subprocess.Popen(command, cwd=self.configuration.repository_root, stdout=self._server_log, stderr=subprocess.STDOUT)
    ...
    def connect_rsp(self, timeout: float = 5.0) -> RSPClient:
        ...
        client = RSPClient.connect(
            ...
            timeout=timeout,
            packet_observer=lambda packet: self._record_rsp_packet(connection_id, packet))
        try:
            ...
            if b"PacketSize=" not in client.command(b"qSupported:PacketSize=4000", timeout=timeout):
    ...
    def connect_stream(self, port: int, artifact_name: str,
        ...
        if not self.is_running:
            raise GDBServerError("cannot connect to a stopped gdbserver")
        return TCPStreamClient.connect("127.0.0.1", port, timeout=timeout, data_observer=lambda data: self._record_stream_data(artifact_name, data))
    ...
    def _is_transient_program_failure(self, output_path: Path) -> bool:
        """Whether the completed load failed with the observed probe USB timeout."""
        return self._TRANSIENT_PROGRAM_FAILURE in self._read_log(output_path)
    ...
    def _wait_for_ready_marker(self, log_path: Path) -> None:
        ...
        while time.monotonic() < deadline:
            ...
            if self._process.poll() is not None:
                raise GDBServerStartError("gdbserver exited with code %d; see %s" % (self._process.returncode, log_path))
            if self._READY_MARKER in self._read_log(log_path):
                return
            time.sleep(0.050)

        raise GDBServerStartError("gdbserver did not become ready within %.1f seconds; see %s" % (self.configuration.start_timeout, log_path))
    ...
    def _write_run_metadata(self, command: Sequence[str]) -> None:
        self.configuration.artifacts.write_json("run.json", {
            ...
            "cbuild_target_type": self.configuration.cbuild_target_type,
            ...
            "pyocd_git_commit": _repository_git_commit(self.configuration.repository_root),
...
def _format_session_option(value: object) -> str:
    ...
    if value is None:
        return "null"
    return yaml.safe_dump(value, default_flow_style=True, sort_keys=False, width=1000000).strip()
