...
class FixtureMailbox:
    ...
    def from_bytes(cls, data: bytes) -> "FixtureMailbox":
        ...
        if len(data) != MAILBOX_SIZE:
            raise FixtureMailboxError("mailbox must be %d bytes, got %d" % (MAILBOX_SIZE, len(data)))
...
class FixtureMailboxClient:
    ...
    def wait_until_ready(self, timeout: float = 5.0, poll_interval: float = 0.050) -> FixtureMailbox:
        ...
        raise FixtureMailboxTimeoutError("fixture mailbox did not initialize: magic=0x%08x abi=%d" % (last_mailbox.magic, last_mailbox.abi_version))
    ...
    def request(self, command: MailboxCommand, argument: int = 0,
        ...
        self._rsp_client.write_memory(self.address + COMMAND_ARGUMENT_OFFSET, struct.pack("<I", argument & 0xffffffff), timeout=timeout)
        self._rsp_client.write_memory(self.address + COMMAND_OFFSET, struct.pack("<I", int(command)), timeout=timeout)
        self._rsp_client.write_memory(self.address + COMMAND_SEQUENCE_OFFSET, struct.pack("<I", sequence), timeout=timeout)
        return sequence
    ...
    def release_spin(self, sequence: int, timeout: float = 5.0) -> None:
        ...
        self._rsp_client.write_memory(self.address + SPIN_RELEASE_SEQUENCE_OFFSET, struct.pack("<I", sequence & 0xffffffff), timeout=timeout)
    ...
    def wait_for(self, predicate: Callable[[FixtureMailbox], bool], *,
        ...
        if last_mailbox is None:
            raise FixtureMailboxTimeoutError("could not read the fixture mailbox")
        raise FixtureMailboxTimeoutError("%s did not occur within %.1f seconds" % (description, timeout))
    ...
    def wait_for_completion(self, sequence: int, timeout: float = 5.0,
        ...
        while time.monotonic() < deadline:
            last_mailbox = self.read(timeout=self._remaining_timeout(deadline))
            if (last_mailbox.completed_sequence == sequence and last_mailbox.result == MailboxResult.COMPLETE):
                return last_mailbox
            time.sleep(min(poll_interval, self._remaining_timeout(deadline)))
        ...
        raise FixtureMailboxTimeoutError("command sequence %d did not complete (completed=%s result=%s)" % (sequence, completed_sequence, result))
