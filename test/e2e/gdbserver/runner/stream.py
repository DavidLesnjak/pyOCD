...
class TCPStreamClient:
    ...
    def connect(cls, host: str, port: int, timeout: float = 5.0,
        ...
        while True:
            try:
                connection = socket.create_connection((host, port), timeout=max(0.001, deadline - time.monotonic()))
                return cls(connection, data_observer=data_observer)
            except OSError as error:
                last_error = error
                if time.monotonic() >= deadline:
                    raise TCPStreamConnectionError("could not connect to TCP stream %s:%d" % (host, port)) from last_error
