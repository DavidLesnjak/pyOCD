...
- CMSIS solution: this directory.
- CubeMX-generated board layer: `Board/B-U585I-IOT02A/`
- Shared test-firmware source and mailbox ABI: `../gdbserver_test_firmware.c`
  and `../gdbserver_test_firmware.h`.
- B-U585I-IOT02A platform adapter: `Source/gdbserver_test_firmware_platform.c`
  and `.h`. It provides retained reset state, delay, WFI wake setup, fault and
  reset requests, and the board's ITM/SWO configuration.
- Built target description: `out/CubeMX+STM32U585AIIx.cbuild-run.yml`
...
- `test_swv.py` decodes test firmware port-0 ITM frames from raw SWV data only when
  the operator enables SWV and supplies this firmware's 160 MHz system clock and
  2 MHz SWO baud with the runner clock options. It covers early and late stream
  attachment, no-GDB
...
hardware-, software-, and literal-BKPT stops during target-assisted range
stepping, watchpoints, execution control, reset, WFI, faults, reconnect,
all-stop, non-stop, and multiple clients. `test_gdb_client.py` is a separate
optional load-and-debug smoke case. All scenarios in `scenarios/arm_gdb/`
require an explicit `--gdbserver-gdb EXECUTABLE`.
