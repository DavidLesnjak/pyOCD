...
- `targets/gdbserver_test_firmware.c` and `.h`: board-independent mailbox ABI,
  commands, and test behavior shared by all target projects.
- `targets/<board>/`: one flattened CMSIS csolution project per physical board.
  Its `Source/gdbserver_test_firmware_platform.*` adapter implements the small
...
```powershell
cbuild test\e2e\gdbserver\targets\b_u585i_iot02a\CubeMX.csolution.yml --context CubeMX.Debug+STM32U585AIIx
```
...
$cbuildRun = 'test\e2e\gdbserver\targets\b_u585i_iot02a\out\CubeMX+STM32U585AIIx.cbuild-run.yml'
...
logs preserve side-channel data. `program-attempt-N.log` preserves every
firmware-programming attempt; the runner retries only the observed transient
probe USB timeout, so permanent programming errors remain a one-attempt failure.

## Explicitly gated scenarios

SWV depends on the physical SWO route, probe support, and the exact clocks used
by the running test firmware. It is skipped unless `--gdbserver-swv` is supplied.
An enabled run must also provide the actual system clock and configured SWO baud;
the runner does not mistake the device's maximum clock for its runtime clock:
